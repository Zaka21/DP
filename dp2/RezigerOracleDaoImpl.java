...
public class ReizigerOracleDaoImpl extends OracleBaseDao implements ReizigersDao {
	...
	public Reiziger save(Reiziger reiziger) throws SQLException {
		...
		PreparedStatement preparedStatement = connection.prepareStatement(
				"INSERT INTO OV.REIZIGER (REIZIGERID, ACHTERNAAM, VOORLETTERS, GEBORTEDATUM) VALUES (?, ?, 'J', ?)");
	...
	public Reiziger update(Reiziger reiziger) throws SQLException {
		...
		PreparedStatement preparedStatement = connection
				.prepareStatement("UPDATE OV.REIZIGER SET ACHTERNAAM = ? AND GEBORTEDATUM = ? WHERE REIZIGERID = ?");
	...
	public boolean delete(Reiziger reiziger) throws SQLException {
		Connection connection = getConnection();
		PreparedStatement preparedStatement = connection
				.prepareStatement("DELETE FROM OV.REIZIGER WHERE REIZIGERID = ?");
		preparedStatement.setInt(1, reiziger.getReizigerId());
		preparedStatement.execute();
		preparedStatement.close();
		connection.close();
	...
	private Reiziger toReiziger(ResultSet resultSet) throws SQLException {
		Reiziger reiziger = new Reiziger(resultSet.getInt("REIZIGERID"), resultSet.getString("ACHTERNAAM"),
